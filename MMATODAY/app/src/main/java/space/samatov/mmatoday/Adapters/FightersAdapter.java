...
import space.samatov.mmatoday.model.LetterGroupHeader;
...
public class FightersAdapter extends RecyclerView.Adapter implements List_Fragment.RecyclerViewDataChangedListener {

    private ArrayList<ListItem> mFighters;
    private boolean mIsUFCList=false;
    private ArrayList<FightersItemClicked> mListeners =new ArrayList<>();
    public FightersAdapter(ArrayList<ListItem> fighters, FightersItemClicked listener, boolean isUFCList){
        mFighters=fighters;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType){
            ...
            case 0:{
                view=LayoutInflater.from(parent.getContext()).inflate(R.layout.fighter_list_item,parent,false);
    ...
    public void notifyListeners(Fighter fighter){
        for (FightersItemClicked listener: mListeners){
            listener.OnFighterListItemSelected(fighter);
        }
    }
    ...
    public class HeaderViewHolder extends ListViewHolder{
        ...
        public void bindView(int position){
            ...
            LetterGroupHeader header= (LetterGroupHeader) item;
    ...
    public class ChampViewHolder extends ListViewHolder {
        ...
        public void bindView(int position) {
            final ListItem item=mFighters.get(position);
            Fighter fighter=(Fighter)item;
            if(fighter.getFirstName().equals("To be determined...")){
                mTextView.setText(fighter.getFirstName());
            }
            else
            mTextView.setText(fighter.getFirstName()+" "+fighter.getLastName());
